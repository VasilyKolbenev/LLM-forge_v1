...
Все эксперименты в llm-forge описываются YAML-конфигами. Конфиг содержит настройки модели, обучения, датасета, LoRA, экспорта и логирования.
...
3. **CLI-переопределения** (`forge train config.yaml key=value`)
