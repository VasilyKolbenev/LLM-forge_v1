...
```bash
forge agent init file-assistant
```
...
```bash
forge agent test configs/agents/file-assistant.yaml
```
...
Observation:
  src/llm_forge/cli.py
  src/llm_forge/config.py
  src/llm_forge/training/sft.py
  src/llm_forge/training/dpo.py
  src/llm_forge/eval/evaluator.py
  src/llm_forge/export/gguf.py
  ... (12 files total)
...
Action: read_file(path="src/llm_forge/config.py", max_lines=30)
...
```bash
forge agent serve configs/agents/file-assistant.yaml --port 8081
```
...
llm-forge поддерживает два режима вызова инструментов:
...
Observation: src/llm_forge/config.py
Thought: Нашёл файл, теперь прочитаю его
Action: read_file(path="src/llm_forge/config.py")
Observation: ...
...
```bash
forge agent test configs/agents/file-assistant.yaml --native-tools
```
