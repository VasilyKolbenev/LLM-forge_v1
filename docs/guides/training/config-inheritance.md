...
В llm-forge конфиги строятся по принципу **слоёв**: базовые дефолты переопределяются более специфичными конфигами, а конкретный эксперимент переопределяет всё.
...
```bash
forge train configs/experiments/my-sft.yaml \
  learning_rate=1e-4 \
  training.epochs=5 \
  lora.r=32
```
...
```bash
forge train config.yaml training.learning_rate=1e-4 dpo.beta=0.2
```
...
!!! tip "Отладка конфига"
    ...
    ```bash
    forge -v train configs/experiments/my-sft.yaml
    ```
