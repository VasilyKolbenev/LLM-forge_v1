...
llm-forge поддерживает два бэкенда для запуска обученных моделей как API-сервиса:
...
# Подключение к локальному серверу llm-forge
client = OpenAI(
...
llm-forge собирает метрики производительности в реальном времени:
